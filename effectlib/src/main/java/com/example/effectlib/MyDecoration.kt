...
class Config(val meBackgroundDrawable: Drawable, val youBackgroundDrawable: Drawable
){

    @Px var groupedMargin: Int=2
    @Px var regulMargin: Int=16
    @Px var groupedCornerRadius: Int=4
    @Px var regulCornerRadius: Int=16
...
class MyDecoration(private val config: Config, meItemViewType: ViewType, youItemViewType: ViewType,val isCornerSame:Boolean) :
    RecyclerView.ItemDecoration(){

    val meItemViewType=meItemViewType.int
    val youItemViewType=youItemViewType.int
    ...
    private val regularCornerRadiusF = config.regulCornerRadius.toFloat()
...
//    private val timer = Timer(256)
    ...
    override fun onDraw(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        for (i in 0 until parent.childCount) {
            ...
            val decView = (holder as DecViewHolder).decView
            ...
            val (x, y) = decView.relativeTo(parent)
//                .let { Pair(it.x.toFloat(), it.y.toFloat() }
            ...
            // apply view bounds
            rectF.set(x, y, x + decView.width, y + decView.height)

            // a single message in a group
            if ((!previousItemTheSameType && !nextItemTheSameType) || isCornerSame) {

                Log.i(TAG," meType "+itemViewType)
    ...
    private companion object {
        ...
        private fun <T> Boolean.ternary(left: T, right: T): T {
            return if (this) left else right
        }
